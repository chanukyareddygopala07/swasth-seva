...
export default function AdminQueuesPage() {
  ...
  const { data: queues, isLoading } = useQuery<QueueSummary[]>({
    queryKey: ["queues", "admin", user?.hospital_id],
  ...
  return (
    <div className="space-y-6">
      <h1 className="flex items-center gap-2 text-xl font-bold">
        <Ticket className="size-5 text-primary" /> Queue management
      </h1>
        ...
        <Card>
          <CardContent className="p-10 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Queues appear here once patients start booking OP visits at your hospital.
            </p>
          </CardContent>
        </Card>
      ) : (
